...
When `top_menu=true`, the first layer of the `pages` argument is used as the top menu (each entry becomes a section). The `pages` section is still interpreted for sidebar navigation within each section.
...
Each entry in the first layer of `pages` must be a `"Section Title" => pages_array` pair, where `pages_array`
follows the same format as the `pages` argument (supporting nested subsections, page titles, etc.).
