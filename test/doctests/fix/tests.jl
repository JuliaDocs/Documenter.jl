...
@eval Main begin
    ...
    const DocTestFixArray_1234 =
        Main.ShowWrap("4×1×1 Array{Int64,3}:\n[:, :, 1] =\n 1\n 2\n 3\n 4")
    const DocTestFixArray_2468 =
        Main.ShowWrap("4×1×1 Array{Int64,3}:\n[:, :, 1] =\n 2\n 4\n 6\n 8")
end
...
function test_doctest_fix(dir)
    ...
    @test normalize_line_endings(index_md) ==
          normalize_line_endings(joinpath(@__DIR__, "fixed.md"))
    @test normalize_line_endings(src_jl) ==
          normalize_line_endings(joinpath(@__DIR__, "fixed.jl"))
end
