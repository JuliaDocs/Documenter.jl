...
@testset "prerender with NodeJS" begin
    ...
    makedocs(;
        ...
        format=Documenter.HTML(highlights=["llvm"],),
    )
    assets = read_assets()
    ...
    for _ = 1:2 # test with and without highlightjs file given
        makedocs(;
            sitename="Prerendering code blocks",
        ...
        @test !occursin("<code class=\"language-julia hljs\">function f()", index)
        @test !occursin(
            "<code class=\"language-julia-repl hljs\">julia&gt; function f()",
            index
        )
        @test !occursin("<code class=\"language-llvm hljs\">;  @ int.jl:87 within", index)
        @test occursin(
            "<code class=\"language-julia hljs\"><span class=\"hljs-keyword\">function</span> f()",
            index
        )
        @test occursin(
            "<code class=\"language-julia-repl hljs\"><span class=\"hljs-meta prompt_\">julia&gt;</span>",
            index
        )
        @test occursin(
            "<code class=\"language-llvm hljs\"><span class=\"hljs-comment\">;  @ int.jl:87",
            index
        )
    ...
    @test_logs (:error, "HTMLWriter: prerendering failed") match_mode = :any begin
        makedocs(;
            sitename="Prerendering code blocks",
            format=Documenter.HTML(prerender=true, node=NodeJS_16_jll.node(),),
        )
    end
    assets = read_assets()
    @test !occursin("'highlight-julia'", assets)
    ...
    @test_throws Base.IOError makedocs(;
        sitename="Prerendering code blocks",
        format=Documenter.HTML(prerender=true, node="nope",),
    )
