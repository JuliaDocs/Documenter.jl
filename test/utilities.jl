...
"""
    @test_string a b
Makes sure that string `a` and `b` match.
"""
macro test_string(a, b)
    source = QuoteNode(__source__)
    :(_test_string($(source), $(esc(a)), $(esc(b))))
end

function _test_string(source, a, b)
    @info "@test_string: $(source.file):$(source.line)"
    if a == b
        Test.record(Test.get_testset(), Test.Pass(:test_string, nothing, nothing, nothing))
    else
        @error "@test_string failed at $(source.file):$(source.line)"
        print(Diff{Lines}(a, b))
        Test.record(Test.get_testset(), Test.Fail(:test_string, nothing, nothing, nothing, source))
    end
end
