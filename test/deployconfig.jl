...
@testset "GitHub Actions deploy configuration" begin
    with_logger(NullLogger()) do
        ...
        withenv(
            ...
            @test Documenter.authenticated_repo_url(cfg) ===
                  "https://github-actions:SGVsbG8sIHdvcmxkLg==@github.com/JuliaDocs/Documenter.jl.git"
        end
        ...
        if get(ENV, "GITHUB_ACTIONS", nothing) == "true" && haskey(ENV, "GITHUB_TOKEN")
            ...
            withenv(
                ...
                @test Documenter.authentication_method(cfg) === Documenter.HTTPS
                @test Documenter.authenticated_repo_url(cfg) ==
                      "https://github-actions:$(ENV["GITHUB_TOKEN"])@github.com/JuliaDocs/Documenter.jl.git"
            end
        ...
        @testset "Woodpecker 0.15.x" begin
            with_logger(NullLogger()) do
                ...
                withenv(
                    ...
                    @test Documenter.authentication_method(cfg) === Documenter.HTTPS
                    @test Documenter.authenticated_repo_url(cfg) ===
                          "https://SGVsbG8sIHdvcmxkLg==@github.com/JuliaDocs/Documenter.jl.git"
                end
...
Documenter.deploy_folder(::CustomConfig; kwargs...) =
    Documenter.DeployDecision(; all_ok=true, subfolder="v1.2.3")
...
@testset "Custom configuration" begin
    with_logger(NullLogger()) do
        ...
        @test (@test_logs (
            :warn,
            r"Documenter\.deploy_folder\(::BrokenConfig; kwargs\.\.\.\) not implemented"
        ) Documenter.deploy_folder(cfg)) == Documenter.DeployDecision(; all_ok=false)
        @test (@test_logs (:warn, r"Documenter could not auto-detect") Documenter.deploy_folder(
            nothing
        )) == Documenter.DeployDecision(; all_ok=false)
    end
end
...
@testset "Autodetection of deploy system" begin
    ...
    withenv("TRAVIS_REPO_SLUG" => nothing, "GITHUB_REPOSITORY" => nothing,) do
        cfg = Documenter.auto_detect_deploy_system()
        @test cfg === nothing
    end
    ...
    withenv("CI" => "woodpecker", "GITHUB_REPOSITORY" => nothing) do
    ...
    withenv("CI" => "drone", "GITHUB_REPOSITORY" => nothing) do
        @test_throws KeyError cfg = Documenter.auto_detect_deploy_system()
    end
    withenv("CI" => nothing, "GITHUB_REPOSITORY" => nothing) do
        cfg = Documenter.auto_detect_deploy_system()
        @test cfg === nothing
    end
end
...
@testset "Remote repository paths" begin
    ...
    @test_throws ErrorException Documenter.user_host_upstream(
        "https://github.com/JuliaDocs/Documenter.jl.git"
    )
