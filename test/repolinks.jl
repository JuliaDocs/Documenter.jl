...
using Documenter:
    Documenter, Remotes, git, edit_url, source_url, MarkdownAST, walk_navpages, expand
...
extrepo_noremote_commit =
    init_git_repo(create_defaultfiles, extrepo_noremote; remote=nothing)
...
@testset "Defaults (no arguments)" begin
    ...
    @test edit_url(doc, joinpath(mainrepo, "foo"); rev="main") ==
          "https://github.com/TestOrg/TestRepo.jl/blob/main/foo"
    @test edit_url(doc, joinpath(mainrepo, "bar", "baz", "qux"); rev=nothing) ==
          "https://github.com/TestOrg/TestRepo.jl/blob/$(mainrepo_commit)/bar/baz/qux"
    @test source_url(doc, RepoLinkTests, joinpath(mainrepo, "foo"), 5:8) ==
          "https://github.com/TestOrg/TestRepo.jl/blob/$(mainrepo_commit)/foo#L5-L8"
    # Directories are fine too, but non-existent local paths are not
    @test edit_url(doc, joinpath(mainrepo, "bar"); rev=nothing) ==
          "https://github.com/TestOrg/TestRepo.jl/blob/$(mainrepo_commit)/bar"
    ...
    @test edit_url(doc, joinpath(subrepo, "foo"); rev=nothing) ==
          "https://github.com/TestOrg/AnotherRepo.jl/blob/$(subrepo_commit)/foo"
    @test edit_url(doc, joinpath(subrepo, "bar", "baz", "qux"); rev=nothing) ==
          "https://github.com/TestOrg/AnotherRepo.jl/blob/$(subrepo_commit)/bar/baz/qux"
    # We also automatically pick up the Git remote of path outside the repo
    @test edit_url(doc, joinpath(extrepo, "foo"); rev=nothing) ==
          "https://github.com/TestOrg/ExtRepo.jl/blob/$(extrepo_commit)/foo"
    @test edit_url(doc, joinpath(extrepo, "bar", "baz", "qux"); rev=nothing) ==
          "https://github.com/TestOrg/ExtRepo.jl/blob/$(extrepo_commit)/bar/baz/qux"
    # But if you don't have the Git origin set, then we error
    @test_throws Documenter.MissingRemoteError edit_url(
        doc,
        joinpath(subrepo_noremote, "foo");
        rev=nothing
    )
    @test_throws Documenter.MissingRemoteError edit_url(
        doc,
        joinpath(subrepo_noremote, "bar", "baz", "qux");
        rev=nothing
    )
    @test_throws Documenter.MissingRemoteError edit_url(
        doc,
        joinpath(extrepo_noremote, "foo");
        rev=nothing
    )
    @test_throws Documenter.MissingRemoteError edit_url(
        doc,
        joinpath(extrepo_noremote, "bar", "baz", "qux");
        rev=nothing
    )
    # And the same applies to external directories
    @test_throws Documenter.MissingRemoteError edit_url(
        doc,
        joinpath(extdirectory, "foo");
        rev=nothing
    )
    @test_throws Documenter.MissingRemoteError edit_url(
        doc,
        joinpath(extdirectory, "bar", "baz", "qux");
        rev=nothing
    )
end
...
@testset "Repo only" begin
    ...
    doc = Documenter.Document(
        ...
        repo=Remotes.GitHub("AlternateOrg", "TestRepo.jl")
    )
    @test edit_url(doc, joinpath(mainrepo, "foo"); rev=nothing) ==
          "https://github.com/AlternateOrg/TestRepo.jl/blob/$(mainrepo_commit)/foo"
    @test edit_url(doc, joinpath(mainrepo, "foo"); rev="main") ==
          "https://github.com/AlternateOrg/TestRepo.jl/blob/main/foo"
    @test edit_url(doc, joinpath(mainrepo, "bar", "baz", "qux"); rev=nothing) ==
          "https://github.com/AlternateOrg/TestRepo.jl/blob/$(mainrepo_commit)/bar/baz/qux"
    @test source_url(doc, RepoLinkTests, joinpath(mainrepo, "foo"), 5:8) ==
          "https://github.com/AlternateOrg/TestRepo.jl/blob/$(mainrepo_commit)/foo#L5-L8"
    # We also automatically pick up the Git remote of a subdirectory
    @test edit_url(doc, joinpath(subrepo, "foo"); rev=nothing) ==
          "https://github.com/TestOrg/AnotherRepo.jl/blob/$(subrepo_commit)/foo"
    @test edit_url(doc, joinpath(subrepo, "bar", "baz", "qux"); rev=nothing) ==
...
@testset "repo for extrepo_noremote" begin
    ...
    doc = Documenter.Document(
        root=joinpath(extrepo_noremote, "docs"),
        repo=Remotes.GitHub("ExtRepoOrg", "TestRepo.jl")
    )
    @test edit_url(doc, joinpath(extrepo_noremote, "foo"); rev=nothing) ==
          "https://github.com/ExtRepoOrg/TestRepo.jl/blob/$(extrepo_noremote_commit)/foo"
    @test edit_url(doc, joinpath(extrepo_noremote, "foo"); rev="main") ==
          "https://github.com/ExtRepoOrg/TestRepo.jl/blob/main/foo"
    @test edit_url(doc, joinpath(extrepo_noremote, "bar", "baz", "qux"); rev=nothing) ==
          "https://github.com/ExtRepoOrg/TestRepo.jl/blob/$(extrepo_noremote_commit)/bar/baz/qux"
    @test source_url(doc, RepoLinkTests, joinpath(extrepo_noremote, "foo"), 5:8) ==
          "https://github.com/ExtRepoOrg/TestRepo.jl/blob/$(extrepo_noremote_commit)/foo#L5-L8"
end
...
@testset "Remotes overrides" begin
    ...
    @test edit_url(doc, joinpath(subrepo, "foo"); rev=nothing) ==
          "https://github.com/AlternateOrg/AnotherRepo.jl/blob/$(subrepo_commit)/foo"
    @test edit_url(doc, joinpath(subrepo, "bar", "baz", "qux"); rev=nothing) ==
          "https://github.com/AlternateOrg/AnotherRepo.jl/blob/$(subrepo_commit)/bar/baz/qux"
    ...
    @test edit_url(doc, joinpath(subrepo_noremote, "foo"); rev=nothing) ==
          "https://github.com/AlternateOrg/NoRemoteSubdir.jl/blob/$(subrepo_noremote_commit)/foo"
    @test edit_url(doc, joinpath(subrepo_noremote, "bar", "baz", "qux"); rev=nothing) ==
          "https://github.com/AlternateOrg/NoRemoteSubdir.jl/blob/$(subrepo_noremote_commit)/bar/baz/qux"
    ...
    @test edit_url(doc, joinpath(extdirectory, "foo"); rev=nothing) ==
          "https://github.com/AlternateOrg/ExtRepo.jl/blob/12345/foo"
    @test edit_url(doc, joinpath(extdirectory, "bar", "baz", "qux"); rev=nothing) ==
          "https://github.com/AlternateOrg/ExtRepo.jl/blob/12345/bar/baz/qux"
end
...
@quietly @test_throws Exception Documenter.Document(
    root=joinpath(mainrepo, "docs"),
    remotes=Dict(extdirectory => Remotes.GitHub("AlternateOrg", "ExtRepo.jl"),)
)
...
@testset "Set repo with remotes" begin
    ...
    doc = Documenter.Document(
        root=joinpath(mainrepo, "docs"),
        remotes=Dict(mainrepo => Remotes.GitHub("AlternateOrg", "AlternateRepo.jl"),)
    )
    @test edit_url(doc, joinpath(mainrepo, "foo"); rev=nothing) ==
          "https://github.com/AlternateOrg/AlternateRepo.jl/blob/$(mainrepo_commit)/foo"
    @test edit_url(doc, joinpath(mainrepo, "foo"); rev="main") ==
          "https://github.com/AlternateOrg/AlternateRepo.jl/blob/main/foo"
    @test edit_url(doc, joinpath(mainrepo, "bar", "baz", "qux"); rev=nothing) ==
          "https://github.com/AlternateOrg/AlternateRepo.jl/blob/$(mainrepo_commit)/bar/baz/qux"
    @test source_url(doc, RepoLinkTests, joinpath(mainrepo, "foo"), 5:8) ==
          "https://github.com/AlternateOrg/AlternateRepo.jl/blob/$(mainrepo_commit)/foo#L5-L8"
    ...
    @test edit_url(doc, joinpath(mainrepo, "foo"); rev=nothing) ==
          "https://github.com/AlternateOrg/AlternateRepo.jl/blob/12345/foo"
    @test edit_url(doc, joinpath(mainrepo, "foo"); rev="main") ==
          "https://github.com/AlternateOrg/AlternateRepo.jl/blob/main/foo"
    @test edit_url(doc, joinpath(mainrepo, "bar", "baz", "qux"); rev=nothing) ==
          "https://github.com/AlternateOrg/AlternateRepo.jl/blob/12345/bar/baz/qux"
    @test source_url(doc, RepoLinkTests, joinpath(mainrepo, "foo"), 5:8) ==
          "https://github.com/AlternateOrg/AlternateRepo.jl/blob/12345/foo#L5-L8"
    @test doc.user.remote == Remotes.GitHub("AlternateOrg", "AlternateRepo.jl")
end
