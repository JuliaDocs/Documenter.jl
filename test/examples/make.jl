...
EXAMPLE_BUILDS = if haskey(ENV, "DOCUMENTER_TEST_EXAMPLES")
    ...
    [
        ...
        "html-repo-nothing",
        "html-repo-error",
        "html-sizethreshold-defaults-fail",
        "html-sizethreshold-success",
        "html-sizethreshold-ignore-success",
        "html-sizethreshold-override-fail",
        "html-sizethreshold-ignore-success",
        "html-sizethreshold-ignore-fail",
        "latex_texonly",
        "latex_simple_texonly",
...
const AT_EXAMPLE_FILES = Dict(
    ("png", :big) =>
        MIMEBytes("image/png", read(joinpath(@__DIR__, "images", "big.png"))),
    ("png", :tiny) =>
        MIMEBytes("image/png", read(joinpath(@__DIR__, "images", "tiny.png"))),
    ("webp", :big) =>
        MIMEBytes("image/webp", read(joinpath(@__DIR__, "images", "big.webp"))),
    ("webp", :tiny) =>
        MIMEBytes("image/webp", read(joinpath(@__DIR__, "images", "tiny.webp"))),
    ("gif", :big) =>
        MIMEBytes("image/gif", read(joinpath(@__DIR__, "images", "big.gif"))),
    ("jpeg", :tiny) =>
        MIMEBytes("image/jpeg", read(joinpath(@__DIR__, "images", "tiny.jpeg"))),
)
...
function withassets(f, assets...)
    ...
    for asset in assets
        isfile(dst(asset)) &&
            @warn "Asset '$asset' present, dirty build directory. Overwriting." src(asset) dst(
                asset
            )
...
htmlbuild_pages = Any[
    "**Home**"=>"index.md",
    "Manual"=>["man/tutorial.md", "man/style.md",],
    hide("hidden.md"),
    "Library"=>["lib/functions.md", "lib/autodocs.md",],
    hide(
        "Hidden Pages" => "hidden/index.md",
        Any["Page X"=>"hidden/x.md", "hidden/y.md", "hidden/z.md",]
    ),
    "Expandorder"=>["expandorder/00.md", "expandorder/01.md", "expandorder/AA.md",],
    "unicode.md",
    "latex.md",
    "example-output.md",
    "fonts.md",
    "issue491.md",
    "linenumbers.md",
    "EditURL"=>["editurl/good.md", "editurl/bad.md", "editurl/ugly.md",],
    "xrefs.md",
    "Outputs"=>["outputs/index.md", "outputs/outputs.md",],
]
    ...
    @quietly withassets(image_assets...) do
        makedocs(;
            ...
            format=Documenter.HTML(;
                assets     = ["assets/favicon.ico", "assets/custom.css", asset("https://example.com/resource.js"), asset("http://example.com/fonts?param=foo", class=:css), asset("https://fonts.googleapis.com/css?family=Nanum+Brush+Script&display=swap", class=:css),],
                prettyurls = true,
...
examples_html_doc = if "html" in EXAMPLE_BUILDS
    ...
    @debug "Controlling variables:" EXAMPLE_BUILDS get(
        ENV,
        "DOCUMENTER_TEST_EXAMPLES",
        nothing
    )
    nothing
end
...
examples_html_meta_custom_doc = if "html-meta-custom" in EXAMPLE_BUILDS
    ...
    html_doc(
        ...
        htmlkwargs=(; edit_link=:commit, description="Example site-wide description."),
        image_assets=(
            "images/logo.png",
            "images/logo.jpg",
            "images/logo.gif",
...
examples_html_mathjax2_custom_doc = if "html-mathjax2-custom" in EXAMPLE_BUILDS
    @info(
        "Building mock package docs: HTMLWriter / deployment build using MathJax v2 (custom URL)"
    )
    html_doc(
        "html-mathjax2-custom",
        MathJax2(
...
examples_html_mathjax3_custom_doc = if "html-mathjax3-custom" in EXAMPLE_BUILDS
    @info(
        "Building mock package docs: HTMLWriter / deployment build using MathJax v3 (custom URL)"
    )
    html_doc(
        "html-mathjax3-custom",
        MathJax3(
...
examples_html_pagesonly_doc = if "html-pagesonly" in EXAMPLE_BUILDS
    ...
    @quietly makedocs(
        ...
        pages=["**Home**" => "index.md", "Manual" => ["man/tutorial.md", "man/style.md",],],
        pagesonly=true,
...
examples_latex_doc = if "latex" in EXAMPLE_BUILDS
    ...
    @quietly makedocs(
        ...
        root     = examples_root,
        build    = "builds/latex",
        pages    = htmlbuild_pages = Any["General"=>["index.md", "latex.md", "unicode.md", hide("hidden.md"),         # The title override tests that we escape headings correctly
        "`@example_output`" => "example-output.md",],         # SVG images nor code blocks in footnotes are allowed in LaTeX
        # "Manual" => [
        #     "man/tutorial.md",
        #     "man/style.md",
        # ],
        hide("Hidden Pages" => "hidden/index.md", Any["Page X"=>"hidden/x.md", "hidden/y.md", "hidden/z.md",]), "Library"=>["lib/functions.md", "lib/autodocs.md",], "Expandorder"=>["expandorder/00.md", "expandorder/01.md", "expandorder/AA.md",],],
        doctest  = false,
        debug    = true,
...
examples_latex_texonly_doc = if "latex_texonly" in EXAMPLE_BUILDS
    ...
    @quietly makedocs(
        ...
        pages    = Any["General"=>["index.md", "latex.md", "unicode.md", hide("hidden.md"), "example-output.md", "linenumbers.md",],         # SVG images nor code blocks in footnotes are allowed in LaTeX
        # "Manual" => [
        #     "man/tutorial.md",
        #     "man/style.md",
        # ],
