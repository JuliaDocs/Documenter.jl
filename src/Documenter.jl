...
export TRAVIS, GITLAB_CI, CIRRUS_CI, DRONE, APPVEYOR
...
"""
    deploydocs(
        ...
        uploader = TRAVIS,
        tag = "" # or "v*.*" if you want a specific tag
    )
...
function deploydocs(;
    ...
    if julia !== nothing
        Base.depwarn("the `julia` keyword argument to `Documenter.deploydocs` is " *
            "removed. Use CI Build Stages for determining from where to deploy instead. " *
