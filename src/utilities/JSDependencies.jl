...
struct RemoteLibrary
    ...
    function RemoteLibrary(
        ...
        return new(name, url, deps, exports)
    end
end
...
struct RequireJS
    ...
    function RequireJS(libraries::AbstractVector, snippets::AbstractVector = Snippet[])
        all(x -> isa(x, RemoteLibrary), libraries) ||
            throw(ArgumentError("Bad element types for `libraries`: $(typeof.(libraries))"))
        all(x -> isa(x, Snippet), snippets) ||
            throw(ArgumentError("Bad element types for `snippets`: $(typeof.(snippets))"))
        r = new(Dict(), [])
...
parse_snippet(filename::AbstractString; kwargs...) =
...
function parse_snippet(io::IO)
    ...
    return Snippet(libraries, arguments, snippet)
end
...
function json_jsescape(args...)
    escapes = ('\u2028' => "\\u2028", '\u2029' => "\\u2029")
    return reduce(replace, escapes, init = JSON.json(args...))
end
