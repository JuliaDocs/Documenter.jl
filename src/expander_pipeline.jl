...
function Selectors.runner(::Type{Expanders.EvalBlocks}, node, page, doc)
    ...
    cd(page.workdir) do
        ...
        for (ex, str) in Documenter.parseblock(
            ...
            try
                ...
                @docerror(
                    doc, :eval_block,
                    """
                    failed to evaluate `@eval` block in $(Documenter.locrepr(doc, page))
                    ```$(x.info)
                    $(x.code)
                    ```
                    """, exception = (err, bt)
        ...
        elseif isa(result, Markdown.MD)
            ...
            @docerror(
                doc, :eval_block, """
                Invalid type of object in @eval in $(Documenter.locrepr(doc, page))
                ```$(x.info)
                $(x.code)
                ```
...
function Selectors.runner(::Type{Expanders.SetupBlocks}, node, page, doc)
    ...
    try
        ...
        @docerror(
            doc, :setup_block,
            """
            failed to run `@setup` block in $(Documenter.locrepr(doc, page))
            ```$(x.info)
            $(x.code)
            ```
            """, exception = (err, bt)
