...
* The MathJax3 setup now uses `tex-svg-full.js` and additionally draws in `mhchem` by default ([#2549]). 
  This new default allows to render chemical arrows as generated by Catalyst.jl without additional 
  setup. It is also consistent with the setup used by Pluto.jl.
