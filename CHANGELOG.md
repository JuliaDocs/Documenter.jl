...
* Allow named `@eval` blocks: such a block shares its execution context with all other `@eval`, `@example`, `@repl` and `@setup` blocks on the same page which use the same name. ([#2074], [#TODO])
